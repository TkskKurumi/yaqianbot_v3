...
def Build(ls, route=None) -> Block:
    ...
    if(isinstance(ls, list)):
        ...
        if(typ.startswith("sub")):
            ...
            return ret
        elif(typ.startswith("source")):
            try:
                with open(ls[1], "r") as f:
                    s = f.read()
                    d = ast.literal_eval(s)
            except Exception as e:
                print("Error reading", ls[1])
                raise e
            ret = Build(d)
            return ret
        else:
...
def get_texts(ls, mx=2048, order=False):
    if(isinstance(ls, list)):
        top = Build(ls)
    ...
    def recur(u: Node, max_branch):
        ...
        idx_to = [(idx, i) for idx, i in enumerate(to)]
        random.shuffle(idx_to)
        rem_leaf = max_branch
        rem_branch = len(idx_to)
        idx_cnt_to = []
        for idx, i in idx_to:
            if (rem_leaf<=0):
                break
            cnt = max(1, ceil(rem_leaf/rem_branch))
            idx_cnt_to.append((idx, cnt, i))
            rem_leaf -= cnt
            rem_branch -= 1
        if (order):
            idx_cnt_to.sort()
        # if (DEBUG and len(idx_cnt_to)>1):
        #     print(max_branch, [cnt for idx, cnt, v in idx_cnt_to])
        for idx, cnt, v in idx_cnt_to:
            recur(v, cnt)
        stack.pop()
        u.revert(S)
    recur(top.head, mx)
    if(len(ret)>mx):
        if (order):
            idx_ret = list(enumerate(ret))
            ret = random.sample(idx_ret, mx)
            ret = [i for idx, i in sorted(ret)]
        else:
